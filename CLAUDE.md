...
- **Granular auto-normalize:** `grammarConfig.autoNormalize` accepts `boolean | TAutoNormalizeConfig`. `true`/`false` enable/disable all normalizations (backward compatible). The object form provides five flags: `wrapInsertFormula` (auto-insert formula buffers in `addExpression`/`insertExpression`/`wrapExpression`), `negationInsertFormula` (auto-insert formula buffer in `toggleNegation` when wrapping a non-not operator in NOT), `collapseDoubleNegation` (collapse NOT(NOT(x)) → x in `toggleNegation`), `collapseEmptyFormula` (auto-collapse operators/formulas after `removeExpression`), and `repositionOnCollision` (auto-redistribute sibling positions when a midpoint-computed position collides during `addExpressionRelative`, `appendExpression`, or `promoteChild`; also controls improved child spacing in `insertExpression`). Post-load normalization in `fromSnapshot`/`fromData` only runs when `autoNormalize === true` (boolean); granular config objects skip post-load normalization.
