...
- **Argument forking:** `forkArgument()` creates an independent copy of the current argument with new UUIDs and `forkedFrom` provenance metadata on every entity (argument, premises, expressions, variables). `canFork()` is a protected overridable method that subclasses use to inject validation policy (e.g., only fork published arguments). Returns `{ engine, remapTable }`. Fork-aware diffing via pluggable entity matchers (`premiseMatcher`, `variableMatcher`, `expressionMatcher` on `TCoreDiffOptions`) and the `createForkedFromMatcher()` helper.
