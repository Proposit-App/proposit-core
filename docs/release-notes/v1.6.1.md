...
- Fixed a related defect where, with the `undici` package installed, the raised-timeout HTTP client was wired incorrectly and made **every** Ollama request fail immediately (`fetch failed`). The timeout client is now paired correctly, and a regression test exercises a real request end-to-end.
