...
Minor release adding a per-attempt LLM-call event to the pipeline
framework so consumers can observe full prompts + raw outputs without
hooking into the `TLlmProvider` interface.
...
including every schema-failed attempt (the retry-about-to-fire ones
**and** the final attempt of a retry-exhausted stage) — carrying the
actual prompts sent (system + user, with the as-sent user message
including any retry-suffix appended on attempt 2+), the raw provider
output, and the call's token usage.

A new optional `validationError` field is set whenever the output failed
`outputSchema` validation, including on the final attempt of a
retry-exhausted stage; it is `undefined` when the schema accepted the
output. The field's presence is the validation-fail signal, not whether
a retry will fire. The payload shape is identical on success and
validation-failure — `validationError` is the only discriminator, so
consumers can write per-attempt rows uniformly.
