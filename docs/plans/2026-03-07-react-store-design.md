...
- `getSnapshot` returns a cached, structurally-shared snapshot. Same reference if nothing has changed.
...
- Add `listeners` set, `subscribe()`, `notifySubscribers()`, `getSnapshot()`
