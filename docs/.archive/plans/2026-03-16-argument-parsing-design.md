...
`TParsedVariable.symbol` maps to the core variable's `symbol` field (the human-readable propositional symbol like "P", "Q"). `TParsedVariable.miniId` is used only for formula operand resolution and cross-referencing within the parsed response — it does not appear on the core variable.
...
5. Wire up claim-source associations via `sourceMiniIds` — resolve miniIds to real claim/source UUIDs and versions from the maps built in steps 3-4; generate a UUID for each association's `id`. Call `mapClaimSourceAssociation()` for extension fields.
6. Create `ArgumentEngine` with the libraries
7. For each parsed variable → `mapVariable()` → `engine.addVariable()` with claim binding. Resolve `claimMiniId` to real `claimId`/`claimVersion` from the map built in step 3. Set `symbol` from `TParsedVariable.symbol`. Build phase generates `id`, `argumentId`, `argumentVersion`, `checksum`.
8. For each parsed premise → `mapPremise()` → `engine.addPremise()`, then use the pre-validated formula AST to add expressions (build phase generates all expression system fields). Note: the first premise added triggers auto-conclusion assignment, which is overridden in step 9.
9. Set conclusion role based on `conclusionPremiseMiniId` (overrides any auto-assignment from step 8)
