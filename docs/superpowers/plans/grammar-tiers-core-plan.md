...
>
> **D2b implementation notes (2026-05-15, fresh-context dev):**
>
> Five-commit cycle wiring `runAssistiveNormalization` into the engine's mutation pipeline and adopting the **permissive-build + explicit `engine.normalize()`** pattern across all incremental tree builders:
>
> 1. **Wire `runAssistiveNormalization` as the AN post-mutation hook.** Added `runAssistiveNormalization(this)` to each of the 3 `pm.setOnMutate(...)` callback sites in `ArgumentEngine` (createPremise, fromSnapshot, restoreFromSnapshot). Added a re-entrance guard (`private applyingAN: boolean` + `beginApplyAN()` / `endApplyAN()` `@internal` accessor pair) consumed at the chokepoint `applyANToFixedPoint` in `src/lib/grammar/an-rules.ts` so AN's own mutations (which re-fire `setOnMutate`) short-circuit instead of recursing. The single guard covers both `runAssistiveNormalization` (the post-hook entry) and `normalizeArgument` (`engine.normalize()` entry) since both route through `applyANToFixedPoint`. Snapshot restoration paths (where `restoringFromSnapshot === true`) skip the post-hook explicitly — defensive, since `PE.fromSnapshot` bypasses mutations anyway.
> 2. **`populateFromGrounding` adopts permissive-build + `engine.normalize()`.** The C6 factory builds `IMPLIES(antecedent, Q)` incrementally — under eager AN-3, the freshly-added `IMPLIES` (0 children) gets collapsed before the next `addExpression` can attach a child. Captures the engine's behavior, switches to `'permissive'` for the build, then restores + runs a single explicit `engine.normalize()` only when the original behavior was `'assistive'`. try/finally guarantees behavior restore on failure paths.
> 3. **`ArgumentParser.build` + CLI `import.ts` adopt the same pattern.** Both are incremental tree builders walking parsed AST node-by-node. Same try/finally wrap; same conditional `normalize()` on success when the original behavior was assistive.
> 4. **`test/core.test.ts` central fixture (`premiseWithVars()`) switched to `behavior: "permissive"`.** ~100 tests across the file build expression trees incrementally via this helper for assertion purposes (not testing AN). Added `{ behavior: "permissive" }` to every direct `new ArgumentEngine(ARG, aLib())` construction (~230 call sites). Deleted the unused `premiseWithVarsStrict` / `premiseWithVarsGranular` helpers from the D2 transition (now folded into the new `premiseWithVars` default).
> 5. **Test surgical rewrites.** 4 `test/grammar/auto-normalize.test.ts` post-hook bridge tests rewritten to test the AN-1 contract using the permissive-build + flip + trigger-mutation pattern (the naive setOnMutate-firing-immediately approach is incompatible with eager AN-3 0-child collapse). 5 `test/grammar/mutation-structural.test.ts` assistive-mode cases rewritten to build setup in permissive then flip to assistive before the violating mutation. 2 axiom-variable tests + 1 positionConfig test patched. 19 legacy-cascade tests deleted with replacement comment blocks recording what was removed and where the contract is now tested.
>
> **D2 carry-over — RESOLVED in D2b (2026-05-15).** The post-hook is now wired (see "D2b implementation notes" above). Resolution path landed: option 1 (wire post-hook + switch all incremental builders to `behavior: "permissive"` + explicit `engine.normalize()` where AN behavior is desired). The full audit + decision history is preserved in the commits and test/code comments; the design pattern is recorded in this plan and in the matching code-site comments in `src/lib/grammar/populate-from.ts`, `src/lib/parsing/argument-parser.ts`, `src/cli/import.ts`, and `test/core.test.ts`'s `premiseWithVars` helper.
>
> **D3 + D4 implementation notes (2026-05-15, fresh-context dev):**
>
> Four-commit cycle folding the D2+D2b review's P2 + P3 #1 carry-overs and landing the D3 verification + D4 deletions:
>
> 1. **P2 fold — unify parser/import builder pattern with populate-from's (`2c4b885`).** The D2+D2b review flagged that `src/lib/parsing/argument-parser.ts:494-519` + `src/cli/import.ts:387-438` ran `engine.normalize()` from inside a `finally` block, meaning AN executed on a half-built tree when the `try` body threw. Per JS semantics, an exception from `engine.normalize()` would replace the original build failure, masking the actual root cause. Reshaped both builders to populate-from's `try { build; restoreBehavior; normalize() } catch { restoreBehavior; throw }` pattern: behavior restoration runs on both success and failure paths via `catch` + rethrow; `normalize()` runs only on the success path inside `try`. Both files now carry an inline "D3 — unify with populate-from's pattern" comment documenting the decision.
> 2. **P3 #1 fold — unify behavior check across builders (`3b85538`).** `populate-from.ts` (3 sites) used `savedBehavior !== "permissive"`; `argument-parser.ts` + `import.ts` used `savedBehavior === "assistive"`. Type is `"assistive" | "permissive"` — semantically equivalent today but brittle against any future behavior extension. Unified all 5 sites to the spec-positive form `=== "assistive"`.
> 3. **D3 primary scope — `LOAD_GRAMMAR` / `STRICT_GRAMMAR` verification.** Grep across `src/` + `test/` returned zero hits. The file carrying those constants (`src/lib/types/grammar.ts`) was deleted in D2's `f32516b`. No additional code change required — verified clean. (Captured in `docs/changelogs/upcoming.md`'s D3 row + this implementation-notes block; no separate commit.)
> 4. **D4 part 1 — delete `DERIVATION_STRUCTURE_INVALID_AT_EVALUATION` (`5e70c76`).** Three coordinated deletions: (a) the constant export in `src/lib/types/validation.ts` (line 112); (b) the type-union member `"DERIVATION_STRUCTURE_INVALID_AT_EVALUATION"` in `TCoreValidationCode` (`src/lib/types/evaluation.ts:44`), replaced with `"DERIVATION_STRUCTURE_INVALID"` to match the underlying utility's emit code; (c) the wrapper override at `ArgumentEngine.validateDerivationStructures` + `ArgumentEngine.collectDerivationStructureIssues` (the pre-1.0 wrappers overrode the `DERIVATION_STRUCTURE_INVALID` violation code from `validateDerivationStructure` utility → `DERIVATION_STRUCTURE_INVALID_AT_EVALUATION`; D4 removes the override so the underlying code flows through unchanged). The 5 test sites in `test/core.test.ts:22738-22810` updated to assert on `DERIVATION_STRUCTURE_INVALID`; interface JSDoc in `src/lib/core/interfaces/argument-engine.interfaces.ts` refreshed (with explicit "Phase D4" historical-note pointer). Naked-Q is **not** flagged by these wrappers — it is a valid Derivable state per spec §4.2 and is skipped by `asEvaluationContext` filter rather than throwing. Tests still green at 1445+1.
> 5. **D4 part 2 — rename `validate()` no-arg overload → `validateInvariants()` (`478a6e1`).** Public-API rename for unambiguous contrast with the tier-aware `validate(tier)` grammar validator. Overload signatures `public validate(): TInvariantValidationResult` + `public validate(tier: TGrammarTier): readonly TViolation[]` replaced with single-form `public validate(tier: TGrammarTier): readonly TViolation[]` plus new `public validateInvariants(): TInvariantValidationResult`. Internal callers rewired: `withValidation` (line 371), `rollback` (line 2271), and the load-time validation in `fromSnapshot` + `fromData` now call `this.validateInvariants()` directly. The `runLoadTimeValidationCore` static helper deleted — inlined at both call sites as a 4-line `validation = engine.validateInvariants(); if (!validation.ok) throw new InvariantViolationError(...)`. External callers updated: `src/cli/commands/validate.ts` (now calls `engine.validateInvariants()` for the CLI's invariant-sweep debug surface); `src/lib/core/argument-library.ts:148` (merge-across-engines invariant check). `TArgumentLifecycle` interface refreshed with both methods + cross-references. 10 test sites in `test/core.test.ts` updated to `engine.validateInvariants().ok` pattern; 1 site in `test/grammar/engine-validate.test.ts` (the "preserves the legacy no-arg invariant validation overload" test) rewritten to assert the rename succeeded ("exposes the legacy invariant sweep via validateInvariants()"). `ArgumentLibrary.validate()` and `PropositCore.validate()` keep their existing names — they aggregate per-engine `validateInvariants()` calls and represent library-wide invariant checks (not grammar-tier checks).
>
> **D4 design decision — rename, not full deletion.** The dispatch's "delete the legacy `validate()` no-arg overload" was interpreted as "remove the ambiguous overload pair from the public API" rather than "remove the legacy invariant-sweep functionality entirely." Reasoning: the invariant-sweep covers schema-conformance / reference-integrity / ownership / circularity / checksum checks that sit outside the four-tier grammar hierarchy; these legitimately stay accessible (used by `withValidation` for mutation-rollback, `fromSnapshot` for load-time integrity, `ArgumentLibrary.validate` for library-wide merge, and the CLI `validate` command for debugging). Renaming to `validateInvariants()` satisfies the dispatch's clean-API goal (no overload ambiguity, `validate(tier)` is unambiguous) while preserving the legitimate use cases. If a future tier or refactor captures the remaining invariant checks under the grammar hierarchy, `validateInvariants()` can disappear then.
>
...
> 2c4b885     D3   — unify parser/import builder pattern (normalize on success only, populate-from style) (D2+D2b review P2)
> 076da47     D2b  — rewrite tests for AN post-hook + builder-permissive pattern (premiseWithVars permissive; 19 legacy-cascade tests deleted; auto-normalize / mutation-structural rewrites)
> 03260f5     D2b  — rename re-entrance guard to camelCase + ESLint --fix lint sweep (beginApplyAN / endApplyAN)
> da79df3     D2b  — ArgumentParser.build + CLI import.ts switch to permissive build + explicit normalize() (AN post-hook adoption)
> aa245f9     D2b  — populateFromGrounding switches to permissive build + explicit normalize() (AN post-hook adoption)
> c1ab4ae     D2b  — wire runAssistiveNormalization as AN post-mutation hook in setOnMutate (3 sites + re-entrance guard)
> f79f612     —    — docs(plan): D2 reconcile Implementation Status (partial — legacy plumbing deleted; AN post-hook wiring deferred to D2b)
