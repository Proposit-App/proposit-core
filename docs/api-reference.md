...
#### Three values in, four values out

A reader assigns one of three values — `true`, `false` or `null` (unknown). Evaluation may report a fourth, `CONTESTED`, when the reader's own assignments run through the steps they granted force a variable both true and false. It is not a shade of unknown: an unknown value is one nobody has spoken to, a contested one has been spoken to twice, in opposite directions.

```typescript
const CONTESTED = "contested"
type TCoreTrivalentValue = boolean | null // what a reader assigns
type TCoreQuadrivalentValue = TCoreTrivalentValue | typeof CONTESTED // what evaluation reports
type TCoreResolvedVariableValues = Record<string, TCoreQuadrivalentValue>
interface TCoreResolvedAssignment {
    variables: TCoreResolvedVariableValues
    operatorAssignments: Record<string, TCoreOperatorAssignment>
}
```

`TCoreExpressionAssignment.variables` stays `TCoreTrivalentValue`, so `CONTESTED` cannot be fed back in as an assignment — that is a compile error, not a runtime check. `TCoreResolvedAssignment` carries what closure produced and is what `PremiseEngine.evaluate` accepts; a reader's assignment is assignable to it, never the reverse. Use `isContested(value)` to narrow.

The four values are Belnap's `FOUR` — each is the set of things we have been told about a proposition: `null` is "told neither", `true` is "told true", `false` is "told false", `CONTESTED` is "told both". Conjunction is told true iff both conjuncts are and told false iff either is; disjunction is dual; negation swaps the two components; `a → b` is `¬a ∨ b` and `a ↔ b` is `(a → b) ∧ (b → a)`, exactly as before. Rows are the left operand:

| `∧`   | T   | F   | N   | B   |     | `∨`   | T   | F   | N   | B   |     | `→`   | T   | F   | N   | B   |     | `↔`   | T   | F   | N   | B   |
| ----- | --- | --- | --- | --- | --- | ----- | --- | --- | --- | --- | --- | ----- | --- | --- | --- | --- | --- | ----- | --- | --- | --- | --- |
| **T** | T   | F   | N   | B   |     | **T** | T   | T   | T   | T   |     | **T** | T   | F   | N   | B   |     | **T** | T   | F   | N   | B   |
| **F** | F   | F   | F   | F   |     | **F** | T   | F   | N   | B   |     | **F** | T   | T   | T   | T   |     | **F** | F   | T   | N   | B   |
| **N** | N   | F   | N   | F   |     | **N** | T   | N   | N   | T   |     | **N** | T   | N   | N   | T   |     | **N** | N   | N   | N   | T   |
| **B** | B   | F   | F   | B   |     | **B** | T   | B   | T   | B   |     | **B** | T   | B   | T   | B   |     | **B** | B   | B   | T   | B   |

(`T` = `true`, `F` = `false`, `N` = `null`, `B` = `CONTESTED`.) Restricted to `{T, F, N}` every table is the strong-Kleene table it has always been, so nothing changes for an argument with no conflict in it.

**Where `CONTESTED` comes from.** Constraint propagation merges what each granted step forces into the variable's current value instead of overwriting it or declining to write. Two steps that force opposite values therefore leave the variable `CONTESTED` rather than letting whichever premise sorted first decide, and the closure reaches the same fixed point whatever order it visits premises in. A reader's own assertion takes part in the merge: assert `A = true`, accept `¬A`, and `A` comes back contested with the accepted `not` named in `variableProvenance[A].contestedBy`.

The propagation rules read a truth _component_ rather than an exact value, which is what makes them monotone and the closure therefore order-independent. Two rules visibly commit to that:

- an accepted `A → B` fires forward whenever `A` is **told true**, so it fires when `A` is `CONTESTED`, and `B` becomes told true;
- an accepted `A ∨ B` derives `B` told true whenever every other disjunct is **told false**, so a `CONTESTED` `A` (which is told false among other things) licenses `B`.

The second is disjunctive syllogism off a contested disjunct — the single inference pattern Belnap's paraconsistency exists to block — and the first is modus ponens for the material conditional, likewise invalid in FDE. **This is a deliberate divergence, and the only one.** Propagation and evaluation answer different questions: evaluation asks what a formula displays and uses the FDE tables above unmodified, while propagation asks what a step the reader _granted_ forces. Refusing to carry a granted step through a conflict would freeze every downstream variable at `null`, showing the reader "unknown" for reasoning they explicitly accepted. The exact-value alternative ("fire only when `A` is exactly `true`") is not monotone in the knowledge order — a variable later becoming contested would retract the derivation — and non-determinism is the defect this design exists to remove.

It is not classical explosion: each rule propagates one specific inference the reader accepted, and never licenses an arbitrary conclusion. But it does mean **a conflict can leave every aggregate reading clean**, because only the told-true component travels forward. Read `contestedVariableIds`, never the aggregates, to find out whether anything is contested.

**Rendering it.** `CONTESTED` must not be shown as a flavour of unknown. It means _your inputs, run through the steps you accepted, force this both true and false_ — a state only the reader can resolve, by changing an assignment or withdrawing an acceptance. `variableProvenance[id].contestedBy` names the steps to point at.
...
| Field                                           | Meaning                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `conclusionTrue`                                | Truth value of the conclusion premise's root. `CONTESTED` when the reader's inputs force it both true and false.                                                                                                                                                                                                                                                                                      |
| `contestedVariableIds`                          | Variable IDs whose value came out `CONTESTED`, sorted. Always present; **not** diagnostics-gated. The only fact that always records a conflict — every other field on this table can read clean while one exists.                                                                                                                                                                                     |
...
| `survivingSupportingPremiseCount`               | Authored supporting premises that were not struck. Derivation premises are excluded — they are engine wiring, not authored support — so this can be `0` for an argument whose supporting premises are all derivations.                                                                                                                                                                                |
| `survivingSupportingPremisesTrue`               | Every surviving _authored_ supporting premise is true; derivation premises are excluded. **Vacuously `true` when the count is `0`** — read the two together, and never read this as "the argument worked". The count is `0` either when the reader struck every authored supporting premise or when the argument has none — which includes an argument whose supporting premises are all derivations. |
...
**Attribution** is computed by intervention followed by fresh derivational closure: the reader's assignment is withheld from the seed and the closure is recomputed from the remaining inputs, never by deleting a provenance tag from an already-derived value. Closure only ever adds information and is the least fixed point of a monotone consequence operator on the knowledge order, so it has exactly one answer and mutually supporting premises cannot certify each other: with `A → B` and `B → A` both accepted and `A` asserted true, withholding `A` derives nothing. The conclusion asks whether its root comes back `true`; an intermediate claim asks whether the **same value** returns.

**Satisfiability and derivation suppression.** `premiseSetSatisfiable` is classical satisfiability over the **surviving** premise set alone, ignoring the reader's assignment entirely — a different question from the four-valued partial evaluation the rest of the pipeline does, and in particular it cannot see a conflict that arises only from the reader's assertions plus the steps they granted (that shows up as `CONTESTED`, below). It is a truth-table walk over the premises, split into groups that share no variable and walked one group at a time, with a ceiling of `SATISFIABILITY_VARIABLE_CEILING` (16) free variables applied to the largest group rather than to their total — above which the answer is `null` ("not determined": do not suppress and do not warn). `null` is also the answer when no row came out all-true and some row could not be settled, since "unsatisfiable" and "unevaluable" are different claims. When it is `false`, constraint propagation is skipped entirely — the conclusion then reports only what the reader asserted, and every provenance entry reads `asserted` or `unassigned`. Striking the contradicting premise can restore both satisfiability and derivation.
...
Claim-type-aware enumeration: the variable enumeration set excludes every **grounded** claim-bound variable — one whose bound claim has `type === "axiomatic"` **or** `type === "citation"`. Their value is fixed at `true` and they are not a free choice. Counts (`numAdmissibleAssignments`, counterexample search space) are computed against the reduced enumeration set, so an argument with `k` claim-bound variables of which `g` are grounded enumerates `2^(k - g)` assignments rather than `2^k`.

**Citations were free choices until v4.0.1 and are given now.** A citation claim reports what a source says, so a failing case that exists only because the source said the opposite is not a fact about this argument. This is the reading the default assignment always used (`D(claim)` seeds citation and axiomatic alike to `true`); `checkValidity` had carved out only the axiomatic half. An argument whose sole countermodel required a cited claim to be false now returns `isValid: true`.

**Validity and evaluation treat a citation differently, deliberately.** `checkValidity` generates its own rows and ignores the reader's assignment entirely, so there a citation is given. `evaluate()` answers the reader's question, where a citation is only _seeded_ `true` by the default assignment and the reader may assign it either way — so `AXIOM_VARIABLE_ASSIGNMENT_FORBIDDEN` still applies to axiomatic-bound variables **only**, and assigning a citation-bound variable remains legal.
...
### `TCoreArgumentEvaluationResult.propagatedVariableValues?: TCoreResolvedVariableValues`
...
```typescript
type TCoreValueOrigin = "asserted" | "derived" | "contested" | "unassigned"
...
interface TCoreVariableProvenance {
    ...
    contestedBy?: TCoreDerivationStep[] // present iff origin === "contested"
}
```

`"asserted"` is reserved for a reader-supplied `true` or `false`; an explicit _unknown_ reads `"unassigned"`, as does a variable nobody supplied and nothing derived. `"contested"` takes precedence over both and is reported whenever the value is `CONTESTED`. Only one **immediate** step is recorded per derived value — walk `fromVariableIds` transitively to reconstruct a chain, rather than storing whole chains and duplicating the graph. A derived value that overrode an explicit unknown is `"derived"`, not a collision: the reader granted the step that produced it.
...
The standalone satisfiability walk `evaluate()` uses. `input` is `{ premises, freeVariableIds, forcedTrueVariableIds? }`: the premises that must come out true together, the variable IDs available to enumerate over, and the IDs pinned `true` in every row (which get no column).

The premises are split into **groups that share no variable**, and each group is walked over its own columns, so the cost is the sum of the groups' truth tables rather than their product. Two premises land in the same group when some variable is reachable from both — reachable, not merely named: an internally premise-bound variable pulls in everything its bound premise reaches, so premises coupled only through one are correctly kept together. A variable no premise can reach gets no column at all, so passing more IDs than the premises use is harmless.

`SATISFIABILITY_VARIABLE_CEILING` bounds the **largest group**, not the total, so an argument with more free variables than the ceiling can still be answered when it decomposes. Returns `null` when a group exceeds the ceiling, and `null` rather than `false` when no row in a group came out all-true and at least one row could not be settled. A `false` from any single group settles the whole set, even beside a group too large to have been walked.

Every reduction is answer-preserving: the result matches a single flat walk over all the variables at once. No SAT-solver dependency — grouping shrinks the input rather than replacing the method, and the ceiling bounds the worst case.

> A premise's evaluation must depend only on the variables it reaches. `TEvaluablePremise.evaluate` states this, and the grouping is why it matters: a premise walked without a variable it secretly consults can report satisfiable for a set that is not.
...
`PipelineConfigurationError.code` is one of `DAG_CYCLE`, `SELF_DEP`, `UNKNOWN_DEP`, `UNKNOWN_STAGE` (thrown by `executeStage` — see below), `DUPLICATE_STAGE_ID`, `GET_OUTSIDE_DEPS`, or `STATUS_OUTSIDE_DEPS`.

### `executeStage(pipeline, stageId, upstream, input, deps)` → `Promise<TExecuteStageResult>` (since v1.11.0)

Runs **one** stage of `pipeline` against a caller-supplied map of its upstream stages' records, without re-running the whole DAG. Built for a durable orchestrator that runs each stage in a separate process/invocation and persists typed stage outputs between stages: to run stage N, hand it stage N's upstream `{ outcome, output? }` records and `executeStage` reproduces the monolithic run's `ctx.get` / `ctx.stageStatus` semantics exactly. `deps` is `TExecuteStageDeps` — the same as `TExecutePipelineDeps` minus `concurrencyLimit` (one stage runs at a time).

- `upstream: Readonly<Record<string, TStageOutcomeRecord>>` — the upstream records keyed by stage id. Each is `{ outcome, output? }` (see `TStageOutcomeRecord`). The caller may pass a **superset**; `executeStage` uses the stage's own `dependsOn` to pick the relevant ones, and defensively drops `output` for any record whose `outcome !== "completed"` so a stale persisted output cannot leak into a skipped/failed dependency.
- `input` is validated + parsed via `Value.Parse(pipeline.inputSchema, input)` and the **parsed** value seeds `ctx.input`, identical to what `executePipeline` seeds (a stage reading `ctx.input` sees the same value it would in the monolithic run). A schema mismatch throws (caller bug).

`executeStage` emits the per-stage events only — `stage:start`, `stage:llm-request`, `stage:llm-response-created`, `stage:llm-call`, `stage:retry`, `stage:end` — and **no `pipeline:*` bookends** (run-level events are the orchestrator's concern). It throws `PipelineConfigurationError` (`UNKNOWN_STAGE`) when `stageId` is not in `pipeline.stages`, and throws a `PipelineConfigurationError` (`GET_OUTSIDE_DEPS` / `STATUS_OUTSIDE_DEPS`) **out directly** when the stage reads a non-dependency — both are caller bugs surfaced rather than swallowed into the result's `failures`. It does **not** decide whether the stage _should_ run given its upstream outcomes: a `failed`/`skipped` required upstream just means `ctx.get` returns `undefined` for it; the "required dep failed → skip this stage" decision belongs to the caller's scheduler.

### `executeFinalize(pipeline, upstream, input, deps)` → `Promise<TExecuteFinalizeResult<TOutput>>` (since v1.11.0)

Runs `pipeline.finalize` against the caller-supplied upstream records — the symmetric counterpart to `executeStage` for the multi-parent finalize step. `input` is parsed via `Value.Parse(pipeline.inputSchema, input)` (same parity guarantee as `executeStage`); the finalize `ctx` is built with `pipeline.finalize.dependsOn` as its allowed-dep set; the required-finalize-dep gate and the `FINALIZE_UNCAUGHT_ERROR` capture match `executePipeline`'s finalize exactly (`output` stays `null` when any required finalize dep is not `completed`, or when finalize itself throws). `executeFinalize` emits **no events** (finalize is not a stage — it has no `stage:*` lifecycle — and there are no `pipeline:*` bookends). It is `async` purely for signature symmetry with `executeStage`; `TPipelineFinalize.run` stays **synchronous** and the wrapper resolves its result. A `ctx.get`-on-non-dep error raised inside finalize is captured as a `FINALIZE_UNCAUGHT_ERROR` failure (output `null`), mirroring `executePipeline` — the throw-out disposition is specific to `executeStage`'s single-stage path.

**Serialize/rehydrate contract.** A stage's serialized form is `{ outcome, output? }` (`TStageOutcomeRecord`) where `output` is the value the stage's `outputSchema` accepts and is **JSON round-trippable**; `output` is present iff `outcome === "completed"`. Rehydration reads the persisted record back into a `TStageOutcomeRecord` and passes it in `upstream`. The pipeline's finalize output is likewise JSON round-trippable (it is persisted + parsed downstream). Because the unit carries the **outcome** alongside the output, a `skipped`/`failed` upstream (no output) is distinguishable from a `completed`-with-`undefined`-output upstream, so both `ctx.get` (output for `completed` only) and `ctx.stageStatus` reproduce monolithic-run semantics across invocations.

### `launchStage(pipeline, stageId, upstream, input, deps, attempt?)` → `Promise<TLaunchStageResult>` (since v1.11.0)

The **launch** half of the launch/complete split for an **LLM-background stage** — for a durable orchestrator that cannot block a single step for an LLM call's full duration. `launchStage` rehydrates `ctx` from `upstream` + the parsed `input`, builds the stage's request from its prompt, and **submits the background response via `deps.submitBackgroundResponse`, returning `{ responseId, status }` WITHOUT awaiting completion** (the caller persists `responseId`, builds a durable hook keyed on it, and suspends; later, after the response completes, it calls `completeStage`).

- `deps.submitBackgroundResponse` is **required** here (a structurally-typed dep — see `TExecuteStageDeps`); `launchStage` throws `PipelineConfigurationError` if it is absent.
- `stageId` must name an **LLM stage** (built by `llmStage`); a deterministic / non-LLM stage throws (it carries no LLM config). Run deterministic stages via `executeStage`.
...
**`completeStage` status/reason → outcome + `retryReason`:**
...
| `incomplete`                          | other / unspecified | `failed`      | `transient`                   |
| `failed`                              | —                   | `failed`      | **none (fail-fast)**          |
| `cancelled`                           | —                   | **`skipped`** | none (no `ProcessingFailure`) |

This mapping is a deliberate `lib/`-side **mirror** of the OpenAI provider's classification (`src/lib/pipelines/` may not import the extension's classifier — the zero-SDK-import boundary); a contract test pins the mirror to the provider so it cannot drift silently. A `cancelled` response settles as `skipped` (not `failed`, no `ProcessingFailure`) so the consumer can distinguish a cancel from an error.
...
| `TStageOutcomeRecord`             | The rehydration unit for one upstream stage: `{ outcome: TStageStatus, output? }`. `output` is present iff `outcome === "completed"` and is JSON round-trippable. Consumed by `executeStage` / `executeFinalize` / `launchStage`.                                                                                                                                                                                                                                        |
| `TExecuteStageDeps`               | `{ llm, generateId?, signal?, onEvent?, submitBackgroundResponse? }`. The `deps` for `executeStage` / `executeFinalize` / `launchStage` / `completeStage`. `submitBackgroundResponse?` is the **submit-only background-response capability** (required by `launchStage`, ignored by the others), referenced by **function type only** so `src/lib/` takes no OpenAI-extension import; the consumer supplies the concrete `extensions/openai`#`submitBackgroundResponse`. |
| `TExecuteStageResult`             | `{ outcome: TStageStatus, output?, failures, tokenUsage?, retryReason? }`. The result of `executeStage` and `completeStage`. `output` is present only when `outcome === "completed"`. `tokenUsage` is present only when the stage both completed AND made an LLM call. `retryReason` (a `TRetryReason` code) is set only by `completeStage` on a RETRYABLE failure — run-to-completion `executeStage` never sets it.                                                     |
| `TExecuteFinalizeResult<TOutput>` | `{ output: TOutput \| null, failures }`. The result of `executeFinalize`; `output` is `null` when a required finalize dep was not completed or finalize threw.                                                                                                                                                                                                                                                                                                           |
| `TLaunchStageResult`              | `{ responseId: string, status: TResponseStatus }`. The result of `launchStage` — the submitted background response's id + submit-time status (may already be terminal on a cached/small submit).                                                                                                                                                                                                                                                                         |
| `TResponseStatus`                 | `"queued" \| "in_progress" \| "completed" \| "failed" \| "incomplete" \| "cancelled"`. The framework-side mirror of the provider's response-status union (structurally identical to the OpenAI extension's `TResponseStatus`).                                                                                                                                                                                                                                           |
| `TRetrievedResponse`              | The framework-side structural shape `completeStage` consumes: `{ status, output?, tokenUsage?, rawResponseId, incompleteReason?, errorMessage? }`. `output` is the **raw assistant text**. The OpenAI extension's `retrieveResponse` returns a structurally-compatible object.                                                                                                                                                                                           |
...
type TRetrievedResponse = {
    ...
    incompleteReason?: string // (since v1.11.0) incomplete_details.reason, when status === "incomplete"
    errorMessage?: string     // (since v1.11.0) error.message, when status === "failed"
}

await retrieveResponse("resp_abc", { apiKey, fetch?, baseUrl?, signal? })
```

Retrieves a stored OpenAI response by id. Throws `ResponseNotFoundError` (HTTP 404) when the response has aged out of the ~10-minute retention window — callers should clear the stored id, settle the stage as failed, and surface a retry. Throws `TransientLlmError` on 5xx or network errors. A 404 surfacing inside the background poll loop also throws `ResponseNotFoundError`. The additive `incompleteReason` / `errorMessage` fields (since v1.11.0) surface `incomplete_details.reason` / `error.message` so a completion-side consumer (`completeStage`) can classify the outcome.
...
#### `submitBackgroundResponse(req, options)` → `Promise<{ responseId, status }>` (since v1.11.0)

```typescript
await submitBackgroundResponse(req, { apiKey, baseUrl?, fetch?, signal? })
// → { responseId: string, status: TResponseStatus }
```

Submits a background OpenAI response (`{ background: true, store: true }`) and returns its `responseId` + submit-time `status` **without polling or streaming to completion** — the submit-only half of the existing `backgroundMode`. It is the provider capability the pipeline's `launchStage` consumes: the caller drives the response to completion later via `retrieveResponse` (typically after a durable suspend keyed on the returned `responseId`). **Terminal-on-submit fast-path:** a small/cached request can come back already terminal on the submit POST — this function still returns `{ responseId, status }` for that case (no throw, no poll; a `cancelled` submit envelope is returned, not thrown, unlike `runBackground`). **No-tools precondition:** a tool-bearing request throws `NonRetryableLlmError`. Throws `TransientLlmError` when the submit envelope carries no id, or on network errors.

The retrieval / lifecycle functions (`retrieveResponse`, `reconnectStream`, `cancelResponse`, `submitBackgroundResponse`) and `TRetrievedResponse` / `TResponseStatus` are exported from the `@proposit/proposit-core/extensions/openai` subpath.
