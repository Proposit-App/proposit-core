...
| `TExecuteStageResult`             | `{ outcome: TStageStatus, output?, failures, tokenUsage? }`. The result of `executeStage`; `output` / `tokenUsage` are present only when `outcome === "completed"`.                                               |
