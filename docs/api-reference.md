...
| `TStageOutcomeRecord`             | The rehydration unit for one upstream stage: `{ outcome: TStageStatus, output? }`. `output` is present iff `outcome === "completed"` and is JSON round-trippable. Consumed by `executeStage` / `executeFinalize`.                                                                          |
| `TExecuteStageDeps`               | `{ llm, generateId?, signal?, onEvent? }`. The `deps` for `executeStage` / `executeFinalize` — `TExecutePipelineDeps` minus `concurrencyLimit`.                                                                                                                                            |
| `TExecuteStageResult`             | `{ outcome: TStageStatus, output?, failures, tokenUsage? }`. The result of `executeStage`. `output` is present only when `outcome === "completed"`. `tokenUsage` is present only when the stage both completed AND made an LLM call — a completed deterministic stage has no `tokenUsage`. |
