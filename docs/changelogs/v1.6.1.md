...
- `src/extensions/ollama/timeout-fetch.ts`: `buildTimeoutFetch` now pairs the raised-timeout undici `Agent` with the **imported undici module's own `fetch`** (`mod.fetch`) instead of handing the Agent to Node's bundled-undici global `fetch` as a `dispatcher`. Node's global `fetch` rejects a foreign-undici `Agent` with `UND_ERR_INVALID_ARG` (`TypeError: fetch failed`), so with `undici` installed every Ollama request failed instantly. Added a deterministic loopback-server regression test that drives a real request through the wrapper (the prior unit tests never fired a request, which is how this shipped).
