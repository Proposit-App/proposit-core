...
- Added `scribe.test.ts` (happy path with a compiled+validated formula; empty-claim → valid `argument: null`; an unresolvable conclusion → `NO_SINGLE_CONCLUSION` processing failure, not a crash; the `argument-ingestion-scribe` wire id; a guard that the `structure` stage prompt carries each claim's title/body, not just its id+type), `reuse-invariant.test.ts` (scribe reuses scholar's four deterministic stage consts by reference and populates the six finalize slots), `export-surface.test.ts` (the two subpath barrels), and `claim-canonicalization-schema.test.ts` (the exported per-extension schema builders); plus a `selectFallbackConclusion` unit test.
- Added `scribe-e2e.test.ts`, a replay-mode golden driver over the five-fixture corpus; its `scribe-recorded-llm.json` + `scribe-expected.json` goldens are recorded against a live `gpt-5.4-mini`, so the suite runs (not inert) and asserts scribe yields a valid argument on each fixture.
- The `structure` stage prompt now includes each canonical claim's content fields (title/body), mirroring scholar's relation-extraction. Previously it passed only `[c1] type=normal` placeholders, so a live model saw no claim text, emitted no relations/conclusion, and scribe degraded to `argument: null` on every multi-claim argument.
