...
- **`normalizeOriginText` (`src/lib/utils/origin-text.ts`) is a one-way door.** Every stored origin anchor is a code-point offset into text it produced, so changing what it emits silently re-indexes all of them — treat an edit as a data migration, not a bug fix. Its step order (line breaks → strip → NFC → trim) is what makes it idempotent. Anchor offsets are code points, never UTF-16 code units: use `sliceByCodePoints`, never `String.prototype.slice`.
