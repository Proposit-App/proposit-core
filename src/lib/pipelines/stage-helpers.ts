...
// (`launchStage`/`completeStage`). None of this is exported from the
// package: there is no consumer use case, and `llmStage`'s public return
// type stays `TStage<TOutput>` (it does NOT widen). The resolved config
// rides an internal-symbol-keyed, non-enumerable carrier on the returned
// stage object, recovered only by the package-internal `readLlmStageConfig`.
